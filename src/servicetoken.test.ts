  ...
  const self: World = {
    ...
    fetch: (async (input, init) => {
      ...
      if (url.startsWith(IDENTITY)) {
        ...
        self.exchanges += 1
        const token = await new SignJWT({ typ: 'service', scopes: ['worlds:read', 'worlds:write', 'billing:read', 'billing:grant'], jti: `t-${++jti}` })
          .setProtectedHeader({ alg: 'RS256', kid: 'k1' })
          .setIssuedAt()
          .setIssuer(ISSUER)
          .setAudience(AUDIENCE)
          .setSubject('service:nda')
          .setExpirationTime(Math.floor(Date.now() / 1000) + SERVICE_TTL_SECONDS)
          .sign(privateKey)
        return new Response(
          JSON.stringify({ token, service: 'nda', scopes: ['worlds:read', 'worlds:write', 'billing:read', 'billing:grant'], expiresIn: SERVICE_TTL_SECONDS }),
