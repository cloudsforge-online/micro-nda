 ...
 * outbox — 13 rows across six topics, none of them registered yet:
 *
 ...
 *
 * The version was not the whole defect. `actor` and `correlation_id` are nullable columns and 11
 * of those 13 rows have a null actor, so the ticket's version-only patch would have left every
 * world event refused for two other reasons. All three are mapped here, at the wire.
 *
...
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    // `system` is the contract's own value for "no principal did this" — a scheduled day roll, which
